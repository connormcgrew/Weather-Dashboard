$(document).ready(function () {
  // my API key was not working so i am using one of my friends API key's
  ...
  function searchWeather(queryURL) {
    $.ajax({
      ...
      if (pastCities[0]) {
        pastCities = $.grep(pastCities, function (savedCities) {
          return id !== savedCities.id;
        });
      }
  ...
  $(document).on("click", "button.city-btn", function (event) {
    ...
    let foundCity = $.grep(pastCities, function (savedCities) {
      return clickedCity === savedCities.city;
    });
